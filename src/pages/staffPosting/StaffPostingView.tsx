...
import { postStaff } from "../../services/schools.service";
import { toast } from "react-toastify";
import { Navbar, Loader } from "components";
import LogoLoader from "../../components/widgets/loader/LogoLoader";
import { cadre } from "pages/updateProfile/DropDownOptions";
...
export const StaffPostingView: React.FC<PostingFormProps> = ({
  schools,
  staff,
  // refreshData
}) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!selectedStaff && !selectedCadre ) {
      toast.error("Please one staff member and cadre.");
      return;
    }
  ...
  const schoolOptions: SelectOption[] = schools
    ...
    const cadreOptions: SelectOption[] = cadre.map((option: any) => ({
      value: option ?? "",
      label: `${option}`
    }));
        ...
        <div className="flex-1 p-6 bg-yellow-500 rounded-lg shadow-lg mt-16">
          ...
          <div className="mb-6">
            ...
            <Select<SelectOption>
              ...
              value={
                cadreOptions.find((option) => option.value === selectedCadre) ?? null
              }
              onChange={handleSelectChange(setSelectedCadre)}
              className="block w-full text-lg border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
